...
class Fetcher():
    ...
    def fetch_search(self, search, results):
        ...
        search = ' '.join(search)
        search = quote_plus(search)  # URL encode unsupported chars
        search_page = browser.get("https://google.com/search?q="+search)

        # find each of the results
        search_list = search_page.soup.find_all('h3', class_='r')

        # get the links from the results
        if search_list:
            for item in search_list:
                if item.parent.a is not None:
            ...
            for item in self.fetched:
                ...
                # dynamically lookup the required class
                # & if no error pass the interface the page.
                try:
                    interface = eval('nani.interfaces.'+lookup+'()')
                    self.results.append(
                        interface.analyze_page(browser.get(item[1]), search))
                    new_fetched.append(item)

                except AttributeError:
                    if self.unsupported:
                        print('No Interface OR Incomplete Interface '+lookup)
                    else:
                        pass
